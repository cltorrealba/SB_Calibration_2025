...
template <class T> class DependentResult;
 ...
 */
template <class T> class CachedResults {
public:
#ifdef IP_DEBUG_CACHE
 ...
 */
template <class T> class DependentResult : public Observer {
public:
#ifdef IP_DEBUG_CACHE
  static const Index dbg_verbosity;
#endif
...
#ifdef IP_DEBUG_CACHE
template <class T> const Index CachedResults<T>::dbg_verbosity = 0;

template <class T> const Index DependentResult<T>::dbg_verbosity = 0;
#endif
...
DependentResult<T>::DependentResult(
    ...
    const std::vector<Number> &scalar_dependents)
    : stale_(false), result_(result), dependent_tags_(dependents.size()),
...
template <class T> DependentResult<T>::~DependentResult() {
#ifdef IP_DEBUG_CACHE
  DBG_START_METH("DependentResult<T>::~DependentResult()", dbg_verbosity);
...
template <class T> bool DependentResult<T>::IsStale() const { return stale_; }

template <class T> void DependentResult<T>::Invalidate() { stale_ = true; }
...
template <class T> const T &DependentResult<T>::GetResult() const {
#ifdef IP_DEBUG_CACHE
...
template <class T> void DependentResult<T>::DebugPrint() const {
#ifdef IP_DEBUG_CACHE
...
template <class T> CachedResults<T>::~CachedResults() {
#ifdef IP_DEBUG_CACHE
  DBG_START_METH("CachedResults<T>::!CachedResults()", dbg_verbosity);
#endif
...
template <class T> void CachedResults<T>::Clear() {
  if (!cached_results_) {
    return;
  }
...
template <class T> void CachedResults<T>::Clear(int max_cache_size) {
  Clear();
  max_cache_size_ = max_cache_size;
}

template <class T> void CachedResults<T>::CleanupInvalidatedResults() const {
#ifdef IP_DEBUG_CACHE
  ...
  while (iter != cached_results_->end()) {
    if ((*iter)->IsStale()) {
      typename std::list<DependentResult<T> *>::iterator iter_to_remove =
          iter++;
...
template <class T> void CachedResults<T>::DebugPrintCachedResults() const {
#ifdef IP_DEBUG_CACHE

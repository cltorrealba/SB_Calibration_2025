...
public:
  ...
  ///@{
  /** Overload if you want the chance to process options or parameters that may
   * be specific to the NLP */
   ...
   *
   *  The Hess_lagrangian_space pointer can be NULL if a quasi-Newton options is
   * chosen.
   */
  virtual bool
  GetSpaces(SmartPtr<const VectorSpace> &x_space,
  ...
  virtual bool GetBoundsInformation(const Matrix &Px_L, Vector &x_L,
                                    ...
                                    const Matrix &Pd_U, Vector &d_U) = 0;
  ...
  virtual bool GetStartingPoint(SmartPtr<Vector> x, bool need_x,
                                ...
                                SmartPtr<Vector> z_U, bool need_z_U) = 0;
  ...
  virtual bool IntermediateCallBack(
      AlgorithmMode /*mode*/, Index /*iter*/, Number /*obj_value*/,
   ...
   *
   * These do not need to be overloaded unless the options are set for user
   * scaling.
   */
  ///@{
  virtual void GetScalingParameters(
    ...
    THROW_EXCEPTION(
        ...
        "You have set options for user provided scaling, but have not "
        "implemented GetScalingParameters in the NLP interface");
  }
  ///@}

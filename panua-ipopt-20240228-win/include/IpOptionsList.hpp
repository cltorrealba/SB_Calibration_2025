...
class IPOPTLIB_EXPORT OptionsList : public ReferencedObject {
  ...
  public:
    ...
    OptionValue(const std::string &value, bool allow_clobber, bool dont_print)
        : value_(value), counter_(0), initialized_(true),
          allow_clobber_(allow_clobber), dont_print_(dont_print) {}

    /** Copy Constructor */
    OptionValue(const OptionValue &copy)
        : value_(copy.value_), counter_(copy.counter_),
          initialized_(copy.initialized_), allow_clobber_(copy.allow_clobber_),
...
public:
  ...
  ///@{
  virtual void
  SetRegisteredOptions(const SmartPtr<RegisteredOptions> reg_options) {

...
const SC_V = 1000.0  # escala flujos
const SC_C = 100.0   # escala sustratos altos (G/F)
...
@constraints(m, begin
    phi1_def, phi1 == sum(FO_L[mc,i] for mc=1:nv, i=1:nfe)
    phi2_def, phi2 == sum(FO_upt[k,i] for k=1:n_up, i=1:nfe)
    phi3_def, phi3 == sum(FO_U[mc,i] for mc=1:nv, i=1:nfe)
end)

@objective(m, Min, FO + omega * (-phi1 + phi2 - phi3))

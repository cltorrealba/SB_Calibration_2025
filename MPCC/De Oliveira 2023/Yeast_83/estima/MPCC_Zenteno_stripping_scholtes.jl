...
UB = log.([1.5*MU0_nom, 1.5*YEG_nom, 1.5*YEF_nom, 1.5*YXN_nom])
...
try
    ...
    @operator(m, op_smooth_injection, 4, smooth_injection)
    @operator(m, op_dynamic_temperature, 1, dynamic_temperature)
    @operator(m, op_death_rate_T, 2, death_rate_T)

    @expression(m, mu0, exp(teta[1]))
    @expression(m, Yeg, exp(teta[2]))
    @expression(m, Yef, exp(teta[3]))
        const Yxg = YXG_nom
        const Yxf = YXF_nom

    @expression(m, T_loc[i=1:ph, j=1:ncp],
        op_dynamic_temperature((i - 1 + radau_nodes[j]) * hv[i])
    )
    @expression(m, mu_T_ij[i=1:ph, j=1:ncp],
        exp(59453.0 * (T_loc[i,j] - 300.0) / (300.0 * R * T_loc[i,j]))
    )
    @expression(m, Kg_T_ij[i=1:ph, j=1:ncp],
        exp(46055.0 * (T_loc[i,j] - 293.15) / (293.15 * R * T_loc[i,j]))
    )
    @expression(m, b_T_ij[i=1:ph, j=1:ncp],
        exp(11000.0 * (T_loc[i,j] - 296.15) / (296.15 * R * T_loc[i,j]))
    )
    @expression(m, mrate_ij[i=1:ph, j=1:ncp],
        0.01 * exp(37681.0 * (T_loc[i,j] - 293.30) / (293.30 * R * T_loc[i,j]))
    )
    @expression(m, mu_j[i=1:ph, j=1:ncp],
        mu0 * mu_T_ij[i,j] * (c[2,i,j] / (c[2,i,j] + Kn0_nom * Kg_T_ij[i,j] + eps))
    )
    @expression(m, betaG_j[i=1:ph, j=1:ncp],
        betaG0_nom * b_T_ij[i,j] *
        (c[3,i,j] / (c[3,i,j] + Kg0_nom * Kg_T_ij[i,j] + eps)) *
        (Kie0_nom * Kg_T_ij[i,j] / (c[5,i,j] + Kie0_nom * Kg_T_ij[i,j] + eps))
    )
    @expression(m, betaF_j[i=1:ph, j=1:ncp],
    ...
    @expression(m, sat_glu[i=1:nfe], c[3,i,1] / (c[3,i,1] + Kg0_nom * Kg_T_ij[i,1] + eps))
    @expression(m, sat_fru[i=1:nfe], c[4,i,1] / (c[4,i,1] + Kf0_nom * Kg_T_ij[i,1] + eps))
    @expression(m, sat_nit[i=1:nfe], c[2,i,1] / (c[2,i,1] + Kn0_nom * Kg_T_ij[i,1] + eps))
    @expression(m, lb_eff[mc=1:nv, i=1:nfe],
        lb[mc] * (
            LB_SELECTOR_GLU[mc] * sat_glu[i] +
            LB_SELECTOR_FRU[mc] * sat_fru[i] +
            LB_SELECTOR_NIT[mc] * sat_nit[i] +
            (1.0 - LB_SELECTOR_GLU[mc] - LB_SELECTOR_FRU[mc] - LB_SELECTOR_NIT[mc])
        )
    )

    @expression(m, phiG_j[i=1:ph, j=1:ncp], c[3,i,j] / (c[3,i,j] + c[4,i,j] + eps))
    @expression(m, phiF_j[i=1:ph, j=1:ncp], c[4,i,j] / (c[3,i,j] + c[4,i,j] + eps))
    @expression(m, Kd_j[i=1:ph, j=1:ncp], op_death_rate_T(c[5,i,j], T_loc[i,j]))
    @expression(m, Yxn, exp(teta[4]))
    @expression(m, injection_rate[i=1:nfe, j=1:ncp],
        op_smooth_injection((i - 1 + radau_nodes[j]) * hv[i], T_INJ_1, DOSE_1, WIDTH_1) +
        op_smooth_injection((i - 1 + radau_nodes[j]) * hv[i], T_INJ_2, DOSE_2, WIDTH_2)
    )

    @expression(m, prod_L[mc=1:nv, i=1:nfe], (v[mc,i]*vs[mc] - lb_eff[mc,i]) * alpha_L[mc,i])
    @expression(m, prod_U[mc=1:nv, i=1:nfe], (ub[mc] - v[mc,i]*vs[mc]) * alpha_U[mc,i])

    @expression(m, prod_L[mc=1:nv, i=1:nfe], (-v[mc,i]*vs[mc] + lb_eff[mc,i]) * alpha_L[mc,i])
    @expression(m, prod_U[mc=1:nv, i=1:nfe], ( v[mc,i]*vs[mc] - ub[mc]      ) * alpha_U[mc,i])
    @expression(m, prod_G[i=1:nfe], (-v[glu,i]*vs[glu]) * alpha_upt[1,i])
    @expression(m, prod_F[i=1:nfe], (-v[fru,i]*vs[fru]) * alpha_upt[2,i])
    
    # Relajacion de Scholtes
    @variable(m, eps_relax >= 0.0)
    fix(eps_relax, MPCC_RELAX_TOL_INIT; force = true)
    @constraints(m, begin
    ...
    @constraints(m, begin
        Lagr[mc=1:nv, i=1:nfe],
            d[mc] + w * v[mc,i] * vs[mc] - alpha_L[mc,i] + alpha_U[mc,i] - up[mc] * alpha_upt[1,i] - up2[mc] * alpha_upt[2,i] +
    ...
    for i in 1:ph, j in 1:ncp
        @constraint(m, cdot[1,i,j] == (mu_j[i,j] - Kd_j[i,j]) * c[1,i,j])
        @constraint(m, cdot[2,i,j] == -(mu_j[i,j] / Yxn) * c[1,i,j] + injection_rate[i,j])
        @constraint(m, cdot[3,i,j] == -((mu_j[i,j] / Yxg) + (betaG_j[i,j] / Yeg) + mrate_ij[i,j] * phiG_j[i,j]) * c[1,i,j])
        @constraint(m, cdot[4,i,j] == -((mu_j[i,j] / Yxf) + (betaF_j[i,j] / Yef) + mrate_ij[i,j] * phiF_j[i,j]) * c[1,i,j])
        @constraint(m, cdot[5,i,j] == (betaG_j[i,j] + betaF_j[i,j]) * c[1,i,j])
    end
...
for (k, eps_val) in enumerate(eps_schedule)
    ...
    fix(eps_relax, eps_val; force=true)
    if k > 1
        set_optimizer_attribute(m, "warm_start_init_point", "yes")
    end
    optimize!(m)
    ...
    # Continuamos la homotopía incluso con ITERATION_LIMIT o ACCEPTABLE_LEVEL, pero no con errores fatales
    if status in (MOI.NUMERICAL_ERROR, MOI.INVALID_MODEL, MOI.INVALID_OPTION)
        println("  [HOMOTOPY] Se detiene la secuencia por error fatal.")
        break
    end
    # Si tenemos LOCALLY_SOLVED, OPTIMAL, ITERATION_LIMIT con punto factible, o ALMOST_LOCALLY_SOLVED, continuamos
    if k < length(eps_schedule) && pr_status == MOI.FEASIBLE_POINT
        println("  [HOMOTOPY] Continuando con punto factible...")
    end
end

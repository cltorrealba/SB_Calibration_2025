    ...
    # Bounds: estimables in [0.1x, 10x] (one log cycle) by default; fixed at nominal (lb=ub)
    ...
    # Optional explicit bounds override via EST_RANGES (multiplicative factors w.r.t. Pnom)
    # Syntax example: EST_RANGES="mu0:0.4,2.5;Yeg:0.1,10"
                        ...
                        try
                            ...
                            if length(lr) >= 2
                                ...
                                if lo !== nothing && hi !== nothing
                                    ...
                                    println("[CFG] EST_RANGES override for ", sname, ": [", lo, ", ", hi, "] x Pnom")
                                end
                            end
                        catch err
                            println("[CFG] WARNING: failed to parse EST_RANGES token '", tok, "': ", err)
                        end
                    end
                end
            end
    ...
    # Override narrower bounds for mu0 only if not explicitly overridden via EST_RANGES
    ...
    # JuMP model
    # ---------------------------------------------
    # Inject Pardiso / Ipopt binary directories into PATH early (Windows DLL search)
    try
        if haskey(ENV, "IPOPT_PARDISO_DLL_DIR")
            pard_dir = ENV["IPOPT_PARDISO_DLL_DIR"]
            if isdir(pard_dir) && !occursin(lowercase(pard_dir), lowercase(get(ENV, "PATH", "")))
                ENV["PATH"] = string(pard_dir, ";", ENV["PATH"])
                println("[CFG] Added pardiso lib dir to PATH: ", pard_dir)
            end
        end
        if haskey(ENV, "PANUA_IPOPT_DIR")
            ipopt_dir = joinpath(ENV["PANUA_IPOPT_DIR"], "bin")
            if isdir(ipopt_dir) && !occursin(lowercase(ipopt_dir), lowercase(get(ENV, "PATH", "")))
                ENV["PATH"] = string(ipopt_dir, ";", ENV["PATH"])
                println("[CFG] Added Ipopt bin dir to PATH: ", ipopt_dir)
            end
        end
    catch err
        @warn "Failed to augment PATH for Pardiso/Ipopt" err
    end
    ...
    begin
        local _pl = try
            parse(Int, get(ENV, "IPOPT_PRINT_LEVEL", "5"))
        catch
            5
        end
        set_optimizer_attribute(m, "print_level", _pl)
        println("[CFG] Ipopt print_level=", _pl)
    end
    ...
    if HOMOTOPY
        ...
        local wall_s_stage = time() - _t0
            ...
            open(metrics_path, "w") do io
                ...
                # Raw Ipopt objective (may be 0 if solver aborted early)
    ...
    if SOLVER_TUNE
        ...
        try
            ...
            post_path = joinpath(RESULTS_DIR, "z_post_" * timestamp * ".png")
            # R² diagnostics post
            if !isempty(t_syn)
                _r2(y_obs, y_pred) = (length(y_obs) <= 1 ? NaN : (1 - sum((y_obs .- y_pred).^2) / sum((y_obs .- mean(y_obs)).^2)))
                # ODE vs DATA
            ...
            # Ensure directory exists prior to saving plot
            try
                mkpath(dirname(post_path))
            catch err
                @warn "Failed to create results directory before post plot save" err
            end

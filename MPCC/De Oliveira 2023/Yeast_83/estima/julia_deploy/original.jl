...
nv=size(S,2)
###############################################
# Project: DC_dFBA (Yeast_83)
# Purpose: JuMP MPCC/dFBA with Radau collocation
# Note: Section headers added for structure parity
###############################################

# =============================================
# 1) Packages and IO setup
# =============================================
nm=size(S,1)
#fluxes constraints
vlb2 = readdlm("lb.csv", ',');
...
ts = pushfirst!(ts,0)
    ...
    # Ipopt attributes (compatible with JuMP v1.x API). On Windows, prefer MUMPS over MA27.
    ...
    # Optional: allow overriding Ipopt max_iter via environment variable for quick reports
    if haskey(ENV, "J_IPOPT_MAX_ITER")
        ...
        @constraints(m, begin
            ...
            alpha1_UB[mc=1:nv,i=1:nfe], alpha_U[mc,i] >= 0   
                        ...
                        # moved FO data-fit equality to NL constraints block below
        ...
        @NLconstraints(m, begin
         ...
         # Nonlinear equality for FO (sum of squared errors)

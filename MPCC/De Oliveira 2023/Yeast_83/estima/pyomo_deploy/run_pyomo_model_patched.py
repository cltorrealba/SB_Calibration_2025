...
def build_model(nfe: int = 12, th: float = 22.0, phi1_val: float = 1.0, phi2_val: float = 1.0, phi3_val: float = 1.0, fix_hv: bool = False):
    ...
    # Fix finite-element durations to constant step h (to mirror JuMP)
    try:
        for _i in m.I:
            m.hv[_i].fix(m.h.value if hasattr(m.h, 'value') else m.h)
    except Exception:
        pass
    ...
    # Nonlinear expressions for uptake rates (evaluate using j=3 collocation point)
    def vg_rule(_m, ii):
        G = _m.c[2, ii, 3]
    ...
    def vz_rule(_m, ii):
        ...
        G = _m.c[2, ii, 3]
        return (
            pyomo_exp(_m.teta[3]) * (Z / (pyomo_exp(_m.teta[4]) + Z)) *
            (1.0 / (1.0 + (G / pyomo_exp(_m.teta[5]))))
        )

    # -- Replace Expressions with explicit Vars + definitions to expose Hessian structure
    m.vg = Var(m.PH, domain=Reals, initialize=lambda _m,ii: 1.0)
    m.vz = Var(m.PH, domain=Reals, initialize=lambda _m,ii: 1.0)

    def VGdef(_m, ii):
        return _m.vg[ii] == vg_rule(_m, ii)
    m.VGdef = Constraint(m.PH, rule=VGdef)

    def VZdef(_m, ii):
        return _m.vz[ii] == vz_rule(_m, ii)
    m.VZdef = Constraint(m.PH, rule=VZdef)
    ...
    # Finite-element durations (kept for compatibility; constraints commented)
    if not fix_hv:
        def mfe1_rule(_m):
...
def solve_and_export(model: ConcreteModel, solver_path: str | None = None, max_iter: int = 5000, print_level: int = 5, use_lbfgs: bool = False, deriv_test: str = "none"):
    ...
    # derivative_test already set above if requested
    t0 = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--deriv_test", dest="deriv_test", type=str, default="none", help="Ipopt derivative_test: none|first-order|second-order (default: none)")

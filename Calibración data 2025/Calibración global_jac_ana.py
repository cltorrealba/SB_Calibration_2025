...
from modelo_dinamico_sim import (
    zenteno_model,
    build_profiles,
    simulate_process_time_stiff,  # << usamos el nuevo
    DEFAULT_X0,
...
def simulate_on_grid(p_real, time_h, temp_segments, pulses, x0=None, method="Radau"):
    """
    Versión que SÍ aplica pulsos instantáneos:
...
def plot_fit_per_assay(p_real: np.ndarray,
    ...
    for i, code in enumerate(kept):
        ...
        ax.plot(td, Xsim[:,0], '-', label="X sim")
        ax.plot(td, Xsim[:,1]*(1/N_SCALE), '-', label="N sim")
        ax.plot(td, Xsim[:,2], '-', label="G sim")
        ax.plot(td, Xsim[:,3], '-', label="F sim")
        ax.plot(td, Xsim[:,4], '-', label="E sim")

        if "biomass_viable_gL" in df: ax.plot(t_meas_d, df["biomass_viable_gL"], 'o', label="X exp")
        if "YAN"               in df: ax.plot(t_meas_d, df["YAN"],               'o', label="N exp")
        if "Glucose"           in df: ax.plot(t_meas_d, df["Glucose"],           'o', label="G exp")
        if "Fructose"          in df: ax.plot(t_meas_d, df["Fructose"],          'o', label="F exp")
        if "Ethanol"           in df: ax.plot(t_meas_d, df["Ethanol"],           'o', label="E exp")

        ax.set_title(f"Ensayo {code}"); ax.set_xlabel("Tiempo (días)")
        ax.set_ylabel("Concentración"); ax.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    FILE_PATH = r"C:/Users/ctorrealba/OneDrive - Viña Concha y Toro S.A/documentos/proyectos i+d/PI-4497/resultados/2025/dFBA/calibración modelo piloto 24_25/calibración data 2025/Procesos_I+D_2025_3.xlsx"   # <-- EDITA si cambia
    ...
    first_code = "SB005"#next(iter(mats.keys()))
    df0 = mats[first_code]
